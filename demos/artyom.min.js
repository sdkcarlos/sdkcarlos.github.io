...
(function (window) {'use strict';
    ...
    var artyomLanguages = {
        ...
        spanish: ["Google español","es-ES","es_ES","es_MX"],
    ...
    // Default voice of artyom without initialization
    var artyomVoice = {
        default:false,
        lang: "en-GB",
        localService: false,
    ...
    function ArtyomInternals() {
         ...
         * @example ArtyomInternals.getVoice(["Inexistent Voice Name","Google Deutsch"]) // Returns German Voice in the browser
         * @param {voiceIdentifier} Array Array of strings with the possible voice identifiers for a language
         * @returns {Voice}
         */
        this.getVoice = function(voiceIdentifiersArray){
            var voice = undefined;
            var voices = speechSynthesis.getVoices();

            for(var i = 0;i < voiceIdentifiersArray.length; i++){
    ...
    function Artyom() {
        ...
        artyom.initialize = function (config) {
            ...
            if (config.hasOwnProperty("lang")) {
                switch (config.lang) {
                    case 'de-DE':
                        artyomVoice = artyomInternals.getVoice(artyomLanguages.german , config.lang);
                        break;
                    case 'en-GB':
                        artyomVoice = artyomInternals.getVoice(artyomLanguages.englishGB , config.lang);
                        break;
                    case "pt-BR":case "pt-PT":
                        artyomVoice = artyomInternals.getVoice(artyomLanguages.brasilian , config.lang);
                        break;
                    case "ru-RU":
                        artyomVoice = artyomInternals.getVoice(artyomLanguages.russia , config.lang);
                        break;
                    case "nl-NL":
                        artyomVoice = artyomInternals.getVoice(artyomLanguages.holand , config.lang);
                        break;
                    case 'es-ES':
                        artyomVoice = artyomInternals.getVoice(artyomLanguages.spanish , config.lang);
                        break;
                    case "en":case 'en-US':
                        artyomVoice = artyomInternals.getVoice(artyomLanguages.englishUSA , config.lang);
                        break;
                    case 'fr':case 'fr-FR':
                        artyomVoice = artyomInternals.getVoice(artyomLanguages.france , config.lang);
                        break;
                    case 'it':case 'it-IT':
                        artyomVoice = artyomInternals.getVoice(artyomLanguages.italian , config.lang);
                        break;
                    case 'jp':case 'ja-JP':
                        artyomVoice = artyomInternals.getVoice(artyomLanguages.japanese , config.lang);
                        break;
                    case 'id':case 'id-ID':
                        artyomVoice = artyomInternals.getVoice(artyomLanguages.indonesia , config.lang);
                        break;
                    case 'pl':case 'pl-PL':
                        artyomVoice = artyomInternals.getVoice(artyomLanguages.polski , config.lang);
                        break;
                    case 'zh-CN':
                        artyomVoice = artyomInternals.getVoice(artyomLanguages.mandarinChinese , config.lang);
                        break;
                    case 'zh-HK':
                        artyomVoice = artyomInternals.getVoice(artyomLanguages.cantoneseChinese , config.lang);
                        break;
                    case 'native':
        ...
        var artyom_talk = function (text, actualChunk, totalChunks, callbacks) {
            ...
            if (artyomVoice) {
                msg.voice = artyomInternals.getVoice([artyomVoice.name, artyomVoice.voiceURI], artyomVoice.lang);
            }
