...
(function(window){'use strict';
    ...
    function ArtyomAI(){
            if(typeof(artyCommands) !== "object"){
                console.warn("Hey .. pss ! You have not loaded the artyom commands file. I can't do anything without it, only (artyom.test())");
            }else{
                ...
                if ('speechSynthesis' in window) {
                    if(config.lang){
                        switch(config.lang){
                            ...
                            break;
                            default:
                                console.info("The given language for artyom is not supported yet. English has been set to default sor.");
                            break;
                        }
                    ...
                    for (var c = 0; c < opciones.length; c++) {
                        ...
                        if(opcion.indexOf("*") != -1){
                            ...
                            if((voz.indexOf(before) != -1) || ((voz.toLowerCase()).indexOf(before.toLowerCase()) != -1)){
                                console.info(">> Matching with a  wildcard | "+ before + " 50%");
                                if((voz.indexOf(later) != -1) || ((voz.toLowerCase()).indexOf(later.toLowerCase()) != -1)){
                                    
                                    //opcion is equal to command
                                    //voz is equal to what you say
                                    var quesito = voz.replace(before,"");
                                        quesito = quesito.replace(later,"");
                                        quesito = quesito.replace(before.toLowerCase(),"");
                                        quesito = quesito.replace(later.toLowerCase(),"");
                                        
                                    var finalFrase = opcion.replace('*',quesito);
